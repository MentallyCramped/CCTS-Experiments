...
vll live_set, var_value;
vvll gwrite, gread;

/*
 * Global var for trans id
 * Every thread has local RS and WS
 * Global WS for live transactions based on the oldest live transaction
 *
 */
...
bool intersect(vll a, vll b){
    ...
    for(int i = 0; i < b.size(); i++){
        if(ht.find(b[i]) != ht.end()) return true;
    }
    ht.clear();
    return false;
}

bool val_write(const sll &RS, const sll &WS, ll tid, const vll &live_set_begin){
    valwritel.lock();
//    printl.lock();
...
//    printl.unlock();
    ...
    for(auto it = RS.begin(); it != RS.end(); it++){
        ...
        if(intersect(live_set_begin, gwrite[rvar])) {
            abort = true;
    ...
    while(true) {
        ...
        while (i < trans.size()) {
            ...
            if (op == 'r') {
                ...
                RS.insert(var);
                localvar[var] = read(var, tid);
            } else {
                WS.insert(var);
                localvar[var] = INT16_MIN;
            }
        }
        bool abort = val_write(RS, WS, tid, live_set_begin);
        if(!abort){
//            printl.lock();
...
//    cout << "All files are done! Average commit time in millisecs " << commit_time/(2000*1000000*10) << endl;
}